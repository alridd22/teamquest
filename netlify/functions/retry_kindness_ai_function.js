// netlify/functions/retry_kindness_ai_function.js
...
function isKindnessActivity(raw) {
  const k = lower(raw);
  return (
    k === "kindness" ||
    k === "the kindness challenge" ||
    k === "kindness challenge"
  );
}
...
module.exports.handler = async (event) => {
  try {
    ...
    for (let r = 1; r < values.length; r++) {
      ...
      const act = row[iAct] || "";
      if (!isKindnessActivity(act)) continue;
    ...
    if (!candidates.length) {
      return ok({
        success: true,
        retried: 0,
        reason: "no stale kindness rows to retry",
      });
    }
    ...
    console.error("retry_kindness_ai_function error:", e);
