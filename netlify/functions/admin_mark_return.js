...
/* ---------- Sheet helpers ---------- */
function lcHeaders(row = []) { const out = {}; (row || []).forEach((h,i)=> out[String(h||'').trim().toLowerCase()] = i); return out; }
async function readTab(sheets, title){
  const r = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `${title}!A:Z` });
  const values = r.data.values || []; return { header: values[0] || [], rows: values.slice(1) };
}
...
async function updateRowPatch(sheets, sheet, rowIndex, headers, patch){
  ...
  for (const [k,v] of Object.entries(patch)){ if (idx[k] != null) row[idx[k]] = v; }
...
/* ---------- In-memory idempotency (warm runtime) ---------- */
const RECENT = new Map(); // key -> expiresAt
function seenRecently(key, ttlMs=60000){
  const now=Date.now();
...
exports.handler = async (event) => {
  try{
    ...
    const idemHeader = (event.headers['x-idempotency-key'] || event.headers['X-Idempotency-Key'] || '').toString().trim();
    const idemKey = idemHeader || `return:${eventId}:${teamCode}:${undo?'undo':'mark'}`;
    if (seenRecently(idemKey)) {
      return ok({ success:true, deduped:true, eventId, teamCode, undo, returnedAt: whenIso, penalty: 0 });
    }
    ...
    if (iEvt == null || iCode == null) return bad(400, 'teams sheet missing Event Id / Team Code headers');

    let rowIdx = -1, current = {};
    for (let r=0; r<tRows.length; r++){
      const row = tRows[r];
      if (((row[iEvt]||'').toString().trim() === eventId) && ((row[iCode]||'').toString().trim() === teamCode)){
        rowIdx = r + 2; // header + 1-based
    ...
    if (!undo){
      try{
        ...
        const eiPPM = eIdx['penaltypermin'] ?? eIdx['penalty per min'] ?? eIdx['penalty/min'] ?? eIdx['penalty'];
      ...
      }catch{ /* tolerate missing events tab */ }
    }

    const has = (name) => tHdr.includes(name);
    const alreadyReturned = !!(String(current['Returned']||'').toUpperCase()==='TRUE' || current['ReturnedAt (ISO)']);
    const desiredReturned = !undo;

    // If already in desired state → no write
    if ((desiredReturned && alreadyReturned) || (!desiredReturned && !alreadyReturned)) {
      const existingPenalty = Number(current['LatePenalty'] || current['Penalty'] || 0) || 0;
      return ok({
        success:true,
        eventId, teamCode,
        returnedAt: desiredReturned ? (current['ReturnedAt (ISO)'] || whenIso) : '',
    ...
    await updateRowPatch(await sheetsClient(), teamSheet, rowIdx, tHdr, patch);
