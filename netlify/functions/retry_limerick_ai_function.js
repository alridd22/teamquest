// netlify/functions/retry_limerick_ai_function.js
...
function isLimerickActivity(raw) {
  const k = lower(raw);
  return (
    k === "limerick" ||
    k === "the limerick challenge" ||
    k === "limerick challenge"
  );
}
...
module.exports.handler = async (event) => {
  try {
    ...
    for (let r = 1; r < values.length; r++) {
      ...
      const act = row[iAct] || "";
      if (!isLimerickActivity(act)) continue;
    ...
    if (!candidates.length) {
      return ok({
        success: true,
        retried: 0,
        reason: "no stale limerick rows to retry",
      });
    }
    ...
    console.error("retry_limerick_ai_function error:", e);
